/*
  # Fix Email Sending to Use SalesPlay's Email Account

  ## Summary
  Fixes critical bug where all emails were sent from the active email account instead of
  the email account selected when creating the salesplay.

  ## Problem
  - User creates Salesplay A with email1@example.com
  - User creates Salesplay B with email2@example.com  
  - All emails sent from whichever account has is_active = true
  - This causes wrong sender emails and breaks multi-account workflows

  ## Solution
  Update process_due_email_tasks() to:
  1. Get email_account_id from the salesplay
  2. Use that specific email account instead of the active one
  3. Fall back to active email account only if salesplay doesn't have one set

  ## Changes
  - Modify email account lookup logic in process_due_email_tasks()
  - Prioritize salesplay.email_account_id over user's active email account
*/
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- CRITICAL FIX: Get email account from salesplay first, fall back to active account
      IF salesplay_record.email_account_id IS NOT NULL THEN
        ...
        LIMIT 1;
      ELSE
        -- Fall back to user's active email account if salesplay doesn't have one
        SELECT * INTO email_account_record
        FROM email_accounts
