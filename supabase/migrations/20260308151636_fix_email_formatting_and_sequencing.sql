/*
  # Fix email formatting - convert newlines to HTML line breaks

  ## Summary
  Email content stored in salesplay_steps uses plain newline characters (\n)
  from the textarea input. When sent as bodyHtml, these newlines are invisible
  in HTML rendering, causing the email to appear as one long run-on paragraph
  without proper spacing or line breaks.

  ## Changes
  1. Converts \n to <br> in the bodyHtml payload before sending to the edge function
  2. Preserves the plain-text version (bodyText) with original newlines intact
  3. No schema changes, only updates the process_due_email_tasks() function
*/
